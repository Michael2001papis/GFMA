@echo off
echo ========================================
echo   GFMA Website - Local Server
echo ========================================
echo.
echo Starting local server...
echo.
echo The website will open at: http://localhost:8000
echo.
echo Press Ctrl+C to stop the server
echo ========================================
echo.

REM Change to project root directory (two levels up from public/tools/)
cd /d %~dp0..\..
cd

REM Try Python 3 first
python --version >nul 2>&1
if %errorlevel% == 0 (
    echo Using Python...
    python -m http.server 8000
) else (
    REM Try Python 2
    python2 --version >nul 2>&1
    if %errorlevel% == 0 (
        echo Using Python 2...
        python2 -m SimpleHTTPServer 8000
    ) else (
        REM Try Node.js http-server
        where http-server >nul 2>&1
        if %errorlevel% == 0 (
            echo Using Node.js http-server...
            http-server -p 8000 -o
        ) else (
            echo ERROR: No server found!
            echo.
            echo Please install one of the following:
            echo   1. Python (https://www.python.org/)
            echo   2. Node.js + http-server (npm install -g http-server)
            echo.
            echo Or simply double-click index.html to open directly in browser
            echo.
            pause
        )
    )
)
